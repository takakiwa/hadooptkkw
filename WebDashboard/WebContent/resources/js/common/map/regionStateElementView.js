	...
	if ($.isArray(regionElement)) {
		for ( var i = 0; i < regionElement.length; i++) {
			regionElement[i].object.mouseover(function(event) {
				// alert(this.regionInfo.time + " " +
				// this.regionInfo.tableName);
				var region = this.regionInfo;
				var now = new Date();
				now.setTime(region.time);
				// var find = new Date();
				// alert(region.tableName);
				var infoString = "tableName:</br>" + region.tableName + "</br>"
...
wgp.RegionStateElementView = Backbone.View
		.extend({
